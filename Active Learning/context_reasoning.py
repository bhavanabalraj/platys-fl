...
def splitByLabels(model, trainData):
    
   
    X_split_dataset = []
    y_split_dataset = []
    
    grouped = trainData.groupby("labels")
    print(grouped)
    
    for i in range(1,7):
    #print(grouped.get_group(6))
        X_split_dataset.append([grouped.get_group(i)[0]])
        y_split_dataset.append([grouped.get_group(i)['labels']])
    ...
    for i in range(0,6):
        ...
        for j in range(len(X_split_dataset[i])):
            transformedData.append(splitAndTransform(X_split_dataset[i][j]))
    
        tensorData = torch.tensor(transformedData)
        X_split_dataset[i] = tensorData
     
    
    
    print(X_split_dataset[0])
