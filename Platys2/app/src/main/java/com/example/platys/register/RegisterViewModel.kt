...
class RegisterViewModel @Inject constructor(
    ...
    private var successScreenShown = false
    ...
    fun registerButtonClicked() {
        ...
        viewModelScope.launch {
            platysRepository.registerUser(email, password).let { result ->
                _dataLoaded.value = Event(false)

                if(result is Result.Success) {
                    _navigationEvent.value = Event(NavigationDestination.REGISTER_SUCCESS)
                    successScreenShown = true
                } else {
                    _navigationEvent.value = Event(NavigationDestination.REGISTER_FAILURE)
                }
            }
        }
    }
    ...
    fun onResume() {
        _clearEmailError.value = Event(Unit)
        _clearPasswordError.value = Event(Unit)
        if(successScreenShown) {
            _navigationEvent.value = Event(NavigationDestination.REGISTER_TO_LOGIN)
        }
    }
}